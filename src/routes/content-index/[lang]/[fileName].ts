...
export async function GET({ params }: APIEvent) {
  const lang = z.enum(["ko", "en"]).safeParse(params.lang).data;
  if (!lang) return new Response(null, { status: 404 });
  const fileName = params.fileName?.replace(".json", "");
  const indexFilesMapping = _indexFilesMapping[lang];
  const mappedSlug =
    fileName &&
    fileName in indexFilesMapping &&
    indexFilesMapping[fileName as keyof typeof indexFilesMapping];
  if (!mappedSlug) return new Response(null, { status: 404 });
  const entryMap = import.meta.glob("~/routes/**/*.mdx", {
    query: "?raw",
  });
  const mdxTable = Object.fromEntries(
    await Promise.all(
      Object.entries(mappedSlug).map(
        async ([title, slug]) =>
