...
const loadNavMenuSystemVersions = cache(async (lang: Lang) => {
  ...
  return calcNavMenuSystemVersions(navMenu["sdk"][lang] || []);
}, "sdk/nav-menu-system-versions");
...
export default function Docs(props: { children: JSXElement }) {
  ...
  const doc = createAsync(() => loadDoc("sdk", fullSlug()), {
    deferStream: true,
  });
  ...
  return (
    <SearchProvider>
      <div class="flex">
        <DocsNavMenu nav="sdk" lang={params().lang} slug={params().slug} />
            ...
            {(frontmatter) => (
              <>
                <Metadata
                  ...
                  ogType="article"
                  ogImageSlug={`sdk/${params().lang}/${params().slug}.png`}
