...
export const route = {
  preload: ({ params }) => {
    ...
    if (section === "overview") {
      void loadOverviewData("v2");
      return;
    }

    void loadCategoryData("v2", section);
  },
...
export default function ApiV2Docs() {
  ...
  const canonicalUrl = createMemo(
    () => `/api/rest-v2/${currentSection()}?v=v2`,
  );
        ...
        {(nav) => (
          <div class="flex gap-5">
            ...
            <Switch>
              ...
              <Match when={currentSection() !== "overview"}>
                <V2Category section={currentSection()} />
              </Match>
            </Switch>
          </div>
        )}
...
function V2Overview() {
  const groups = createAsync(() => Promise.resolve(loadOverviewData("v2")), {
    deferStream: true,
  });
    ...
    <Show when={groups()}>
      {(groups) => (
        ...
        >
          ...
          <prose.p>
            API 매개 변수 중 URL 경로 또는 query에 들어가는 문자열 값이 있는
            경우, 그 자리에 들어갈 수 없는 문자는 이스케이프하여야 합니다.
          ...
          <prose.p>
            GET 요청 시에 body(content)를 전달하는 것은 HTTP 표준에 부합하지
            않아, 클라이언트에 따라 사용 불가능한 경우가 있습니다.
          </prose.p>
          <prose.p>
            이 경우, body 문자열을 <code>requestBody</code> query 필드에 넣어
            주시면 됩니다. query 필드에 들어가는 문자열은 URL 인코드하셔야
            합니다.
          </prose.p>
          <prose.h3 id="idempotency-key">멱등 키</prose.h3>
          <prose.p>
            <code>Idempotency-Key</code> 헤더는 네트워크 장애나 타임아웃 시
            동일한 요청이 중복 처리되는 것을 방지합니다. 자세한 표준은{" "}
          ...
          <prose.p>
            ...
            합니다. 16~256자의 ASCII 문자열(권장: 영문 대소문자, 숫자, -, _) 을
            사용 가능합니다. RFC 8941에 따라 문자열을 쌍따옴표로 감싸세요.
          </prose.p>
          <ul>
            <li>
          ...
          <prose.p>
            재시도에서 <code>IDEMPOTENCY_OUTSTANDING_REQUEST</code> 에러가
            발생한 경우, 시간이 조금 지난 후 다시 재시도해 주세요.
          </prose.p>
          <prose.ul>
          ...
          <prose.p>
            서버에서 요청 처리 중에 클라이언트 측에서 연결을 끊은 경우에도
            요청이 취소되지 않습니다. 이 경우 같은 멱등 키로 같은 요청을
            재시도하면 결과를 확인할 수 있습니다.
          </prose.p>
...
function V2Category(props: { section: string }) {
  const navigate = useNavigate();
  const sectionDescriptionProps = createSectionDescriptionProps({});
  const data = createAsync(() => loadCategoryData("v2", props.section), {
    deferStream: true,
  });
  ...
  createEffect(() => {
    ...
    if (!redirect) return;
    void navigate(`/api/rest-v2/${redirect}`, { replace: true });
  });
