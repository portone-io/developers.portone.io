...
import { getEmptyCategoryIds } from "~/layouts/rest-api/schema-utils/endpoint";
import { loadV2Schema } from "~/layouts/rest-api/schema-utils/load-schema";
...
export default function ApiV2Docs() {
  const schema = createAsync(() => loadV2Schema(), { deferStream: true });
            ...
            <Switch>
              <Match when={currentSection() === "overview"}>
                ...
                >
                  ...
                  <prose.p>
                    API 매개 변수 중 URL 경로 또는 query에 들어가는 문자열 값이
                    있는 경우, 그 자리에 들어갈 수 없는 문자는 이스케이프하여야
                    합니다. 자바스크립트의 <code>encodeURIComponent</code> 함수
                    등을 사용할 수 있습니다.
                    {/* TODO: 안전한 paymentId 사용에 대해 설명 */}
                  </prose.p>
                  <prose.h3 id="auth">인증 방식</prose.h3>
                  <prose.p>
                    V2 API를 사용하기 위해서는 V2 API Secret이 필요하며, 포트원
                    관리자콘솔 내 결제연동 탭에서 발급받으실 수 있습니다.
                  </prose.p>
                  <prose.p>
                    인증 관련 API를 제외한 모든 API는 HTTP{" "}
                    <code>Authorization</code> 헤더로 아래 형식의 인증 정보를
                    전달해주셔야 합니다.
                  </prose.p>
                  <ul>
                    <li>
                  ...
                  <prose.p>
                    GET 요청 시에 body(content)를 전달하는 것은 HTTP 표준에
                    부합하지 않아, 클라이언트에 따라 사용 불가능한 경우가
                    있습니다.
                  </prose.p>
                  <prose.p>
                    이 경우, body 문자열을 <code>requestBody</code> query 필드에
                    넣어 주시면 됩니다. query 필드에 들어가는 문자열은 URL
                    인코드하셔야 합니다.
                  </prose.p>
                  <prose.h3 id="idempotency-key">멱등 키</prose.h3>
                  <prose.p>
                    <code>Idempotency-Key</code> 헤더는 네트워크 장애나 타임아웃
                    시 동일한 요청이 중복 처리되는 것을 방지합니다. 자세한
                    표준은{" "}
                  ...
                  <prose.p>
                    요청 헤더에 고유한 키를 포함하세요. UUID 등 고유한
                    문자열이어야 합니다. 16~256자의 ASCII 문자열(권장: 영문
                    대소문자, 숫자, -, _)을 사용 가능합니다. RFC 8941에 따라
                    문자열을 쌍따옴표로 감싸세요.
                  </prose.p>
                  <ul>
                    <li>
                  ...
                  <prose.p>
                    요청 타임아웃이 발생한 경우, 동일한 멱등 키를 사용하여
                    재시도할 수 있습니다. 이 경우, 서버는 요청을 중복 처리하지
                    않고, 기존 응답을 반환합니다.
                  </prose.p>
                  <prose.p>
                    재시도에서 <code>IDEMPOTENCY_OUTSTANDING_REQUEST</code>{" "}
                    에러가 발생한 경우, 시간이 조금 지난 후 다시 재시도해
                    주세요.
                  </prose.p>
                  <prose.ul>
                    <li>
                  ...
                  <prose.p>
                    API 호출시 PG 및 결제 원천사의 응답 지연을 고려하여 최소
                    60초의 읽기 타임아웃 시간을 설정하도록 권장합니다.
                  </prose.p>
                  <prose.p>
                    서버에서 요청 처리 중에 클라이언트 측에서 연결을 끊은
                    경우에도 요청이 취소되지 않습니다. 이 경우 같은 멱등 키로
                    같은 요청을 재시도하면 결과를 확인할 수 있습니다.
                  </prose.p>
