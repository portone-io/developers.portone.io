...
<Hint style="info">
  🔔 **웹훅(Webhook)** 은 특정 이벤트가 발생했을 때 설정된 URL로 이벤트 데이터를 전송하는 방식입니다.
</Hint>

포트원 파트너 정산 내 세금계산서 제품에서는 세금계산서의 상태가 변경되었을 때 고객사가 사전에 설정한 URL로 즉각적으로 이벤트를 전달하여 효율적인 시스템 연동을 지원합니다.
...
- **확장성**: 여러 시스템이나 서비스를 쉽게 연동할 수 있어 비즈니스 프로세스 자동화와 확장이 용이합니다.

## ✔️ 세금계산서 웹훅 사용 예시

### 📌 사용 시나리오

- 세금계산서가 국세청에 성공적으로 전송 완료된 경우에만 자동으로 정산금 지급 처리를 수행합니다. 
이 때 세금계산서 상태를 확인하기 위해 반복하여 콘솔에 접속하지 않고도 웹훅을 통해 실시간으로 상태 알림을 전달받을 수 있습니다.

- 고객사의 내부 데이터베이스에 세금계산서의 상태를 직접 관리하고자 하는 경우 상태 변경을 내부 시스템에 실시간으로 반영합니다.

### 📌 동작 흐름

1. 포트원에서 특정 이벤트(예: 세금계산서 발행 완료)가 발생합니다.
2. 고객이 설정한 웹훅 URL로 JSON 데이터를 포함한 HTTP POST 요청이 발송됩니다.
3. 고객 서버는 웹훅 데이터를 받아 필요한 내부 로직을 처리합니다.
...
| timestamp | String(ISO 8601) | O | 이벤트 발생 시각 | `2025-03-20T14:25:10Z` |
...
- `DRAFTED`: 임시 저장 완료
- `DRAFT_PENDING`: 임시 저장 대기
- `DRAFT_FAILED`: 임시 저장 실패
- `REQUESTED`: 발행 요청 완료
- `REQUEST_PENDING`: 발행 요청 대기
- `REQUEST_FAILED`: 발행 요청 실패
- `REQUEST_CANCELLED`: 공급받는 자에 의한 발행 취소
- `REQUEST_REFUSED`: 발행 요청 거부
- `ISSUED`: 발행 완료
- `ISSUANCE_CANCELLED`: 공급자에 의한 발행 취소
- `BEFORE_SENDING`: 전송 전
- `WAITING_SENDING`: 전송 대기
- `SENDING`: 전송 중
- `SENDING_COMPLETED`: 전송 완료
- `SENDING_FAILED`: 전송 실패

## ✔️ 웹훅 설정 방법

포트원 세금계산서 웹훅은 포트원 기술팀에서 설정합니다. 아래 절차를 따라 요청해주세요.

### 📌 설정 요청 절차

1. 웹훅 수신 URL을 준비해주세요.
2. 기술지원 이메일(support.b2b@portone.io)로 웹훅 URL(운영 환경, 테스트 환경 별도 적용 가능)을 전달해주세요.
3. 설정이 완료되면 웹훅 정상 작동 여부를 확인하신 뒤 운영 환경에 적용해주세요.

### 📌 웹훅 정상 작동 확인 방법

- 포트원 콘솔에서 웹훅 테스트 토글을 활성화해주세요.
- 테스트로 세금계산서를 발행 요청하면 설정된 URL로 웹훅이 전송됩니다.

### 📌 IP 필터링 안내

포트원은 아래 고정 IP를 사용하여 웹훅을 전송합니다.

안정적 웹훅 수신을 위해 IP를 필터링이 필요하신 경우, 아래 IP를 허용 목록에 추가해 주세요.

- **포트원 V2 웹훅 IP**: `52.78.5.241`

추후 IP 추가/변경 시 사전에 메일로 안내드릴 예정이며, 해당 경우에는 IP 필터를 업데이트해 주셔야 정상적으로 웹훅 수신이 가능합니다.

### 📌 웹훅 실패 시 재시도 정책

웹훅 요청 시 실패 응답을 받은 경우, 포트원에서는 총 5회(최초 요청 + 4회 재시도)까지 웹훅 전송을 시도합니다. 고객사의 시스템 장애 상황에서 웹훅 과부하로 인한 영향을 최소화하기 위해 다음과 같은 재시도 정책을 적용하고 있습니다:
