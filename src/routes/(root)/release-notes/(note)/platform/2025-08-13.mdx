...
일괄지급 내역 상세 및 지급 내역 관리 페이지에서 원하는 정산 내역을 단건, 혹은 다건 으로 선택하여 한 번에 정산내역서 이메일을 발송할 수 있습니다.

**발신자/수신자 설정** : 발신자 이메일 설정은 필수이며, 도메인 인증을 통해 도메인을 활용한 발신이 가능합니다. 인증 완료된 도메인이 없는경우, 기본 메일 [no-reply@portone.io](mailto:no-reply@portone.io) 로 정산내역서를 발송할 수 있습니다. 
...
<strong>지급 내역</strong> `PlatformPayout` <strong>필드 추가</strong>
...
<strong>지급 내역 필터</strong> `PlatformPayoutFilterInput` <strong>옵션 추가</strong>
...
- 기술지원 문의: [support.b2b@portone.io](mailto:support@portone.io) / 파트너스 슬랙 채널
