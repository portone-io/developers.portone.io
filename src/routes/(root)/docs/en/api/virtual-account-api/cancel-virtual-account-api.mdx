---
title: Cancel virtual account API
description: ''
---
...
<Swagger method="delete" path="/vbanks/{imp_uid}" baseUrl="https://api.iamport.kr" summary="Cancels a virtual account before payment is deposited.">
  <SwaggerDescription>
    You can cancel a virtual account before deposit to prevent the buyer from depositing by mistake.

    **imp\_uid**

    must be specified. (Since there can be multiple virtual accounts pending payment deposit for a single merchant\_uid, you can only cancel a virtual account based on the imp\_uid)
  </SwaggerDescription>
  ...
  <SwaggerResponse status="400: Bad Request" description="Missing imp_uid">
    ```ts
    {
      // Response
    }
