---
title: Get payments by order ID, status (All)
description: ''
---
...
<Swagger method="get" path="/payments/findAll/{merchant_uid}/{payment_status}" baseUrl="https://api.iamport.kr" summary="Gets details of payments by order ID, status.">
  <SwaggerDescription>
  ...
  <SwaggerParameter name="payment_status" type="String">
    **Status code**
  ...
  <SwaggerResponse status="200: OK" description="Sucess">
    <Tabs>
      <Tabs.Tab title="PaymentListResponse">
    ...
    <Tabs>
      <Tabs.Tab title="PagedPaymentAnnotation">
  ...
  <SwaggerResponse status="400: Bad Request" description="Parameters errors, such as missing merchant_uid or invalid payment_status">
    ```ts
    {
      // Response
    }
  ...
  <SwaggerResponse status="404: Not Found" description="Invalid merchant_uid">
    ```ts
    {
      // Response
    }
...
>
> - started : Sort in ascending order (ASC)
>   based on payment start time (payment window open time)
>
