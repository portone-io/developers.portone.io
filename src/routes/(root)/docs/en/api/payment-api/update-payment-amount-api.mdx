---
title: Update payment amount API
description: ""
---

import Details from "~/components/gitbook/Details";
import Swagger from "~/components/gitbook/swagger/Swagger";
import SwaggerDescription from "~/components/gitbook/swagger/SwaggerDescription";
import SwaggerParameter from "~/components/gitbook/swagger/SwaggerParameter";
import SwaggerResponse from "~/components/gitbook/swagger/SwaggerResponse";
import Tabs from "~/components/gitbook/Tabs";
import Hint from "~/components/Hint";

## Updates the amount saved via [**Save payment amount API**](../save-payment-amount-api).

<Swagger method="put" path="/payments/prepare" baseUrl="https://api.iamport.kr" summary="Updates the pre-saved amount.">
  <SwaggerDescription />

  ### Parameters

  #### Body

  <SwaggerParameter name="merchant_uid" type="String" required="true">
    <mark style="color:red;">
      **Merchant order ID**
    </mark>
  </SwaggerParameter>

  <SwaggerParameter name="amount" type="double" required="true">
    <mark style="color:red;">
      **Payment amount**
    </mark>
  </SwaggerParameter>

  ### Responses

  <SwaggerResponse status="200: OK" description="Success">
    <Tabs>
      <Tabs.Tab title="PaymentPrepareResponse">
        **`code`** <mark style="color:red;">**\***</mark> <mark style="color:purple;">**integer**</mark>

        **Response code**

        0: success, Not 0: check the message

        **`message`** <mark style="color:red;">**\***</mark> <mark style="color:green;">**string**</mark>

        **Response message**

        A non-zero code includes a message like 'Invalid payment info'.

        <mark style="color:red;">
          **`response (PaymentPrepareAnnotation, optional)`**
        </mark>
      </Tabs.Tab>
    </Tabs>

    <Tabs>
      <Tabs.Tab title="PaymentPrepareAnnotation">
        **`merchant_uid`` `**<mark style="color:red;">**`*`**</mark>**` `**<mark style="color:green;">**`string`**</mark>

        **`Merchant order ID`**

        **`amount`**<mark style="color:red;">**`*`**</mark><mark style="color:orange;">**`number`**</mark>

        **`Payment amount`**
      </Tabs.Tab>
    </Tabs>
  </SwaggerResponse>

  <SwaggerResponse status="401: Unauthorized" description="Missing or invalid access token">
    ```ts
    {
      // Response
    }
    ```
  </SwaggerResponse>

  <SwaggerResponse status="404: Not Found" description="Invalid merchant_uid">
    ```ts
    {
      // Response
    }
    ```
  </SwaggerResponse>
</Swagger>

<Details>
  <Details.Summary>Response Model Schema</Details.Summary>

  <Details.Content>
    ```json
    {
      "code": 0,
      "message": "string",
      "response": {
        "merchant_uid": "string",
        "amount": 0
      }
    }
    ```
  </Details.Content>
</Details>

<Hint style="success">
  **Swagger Test Link**

  [**https://api.iamport.kr/#!/payments.validation/editPreparePayment**](https://api.iamport.kr/#!/payments.validation/editPreparePayment)
</Hint>
