---
title: Get payments by order ID, status (Top 1)
description: ''
---
...
<Swagger method="get" summary="Gets details of payments by order ID, status. This returns top 1  transaction based on the sort order." baseUrl="https://api.iamport.kr" path=" /payments/find/{merchant_uid}/{payment_status}">
  <SwaggerDescription>
    If you specify the payment\_status, this returns the latest data for the status.
  </SwaggerDescription>
  ...
  <SwaggerParameter type="String" name="payment_status">
    **Status code**
  ...
  <SwaggerResponse status="404: Not Found" description="Invaliud merchant_uid">
    ```ts
    {
      // Response
    }
