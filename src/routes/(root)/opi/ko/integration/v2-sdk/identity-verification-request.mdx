...
targetVersions: ["v2"]
versionVariants:
  v1: /ko/integration/sdk/javascript-sdk/cft
---
...
<ParamTree>
  ...
  - **`inicis`** <mark style="color:blue;">**object**</mark>

    **KG이니시스 bypass 파라미터**

    <ParamTree>
      - **`reqSvcCd`** <mark style="color:green;">**string**</mark>

        **KG이니시스 통합인증에서 사용할 서비스 코드**

        - 간편인증의 경우 `01` 입력.
        - 본인확인의 경우 `03` 입력.

      - **`DI_CODE`** <mark style="color:green;">**number**</mark>

        **DI를 생성할 때 사용할 웹사이트 코드**

        본인확인 요청의 경우에 이 값을 채워 넣어서 DI를 생성할 때 전달할 수 있습니다.
      ...
      - **`directAgency`** <mark style="color:green;">**string**</mark>
        ...
        **단독 노출할 인증 업체 코드**
        ...
        <Details>
          <Details.Summary>가능한 인증 업체 코드와 서비스 코드 조합</Details.Summary>

          <Details.Content>
            |인증 업체 명칭|인증 업체 코드|`01` (간편인증)|`03` (본인확인)|
            |:------------:|:------------:|:-------------:|:-------------:|
            |    페이코    |     PAYCO    |      지원     |     미지원    |
            | 패스(통신사) |     PASS     |      지원     |     미지원    |
            |     토스     |     TOSS     |      지원     |      지원     |
            ...
            |   우리은행   |     WOORI    |      지원     |      지원     |
            |   농협은행   |      NH      |      지원     |     미지원    |
            |  카카오뱅크  |   KAKAOBANK  |      지원     |     미지원    |
            |  휴대폰 인증 |      SMS     |      지원     |      지원     |
          </Details.Content>
        </Details>

      - **`flgFixedUser`** <mark style="color:green;">**string**</mark>

        **인증 창에서 고객 정보를 미리 채울지 여부**

        `Y`, `N` 중 하나를 입력해주세요.

        `Y`인 경우 구매자 이름, 구매자 연락처, 구매자 출생년도, 구매자 출생월, 구매자 출생일을 필수로 입력해야 합니다.
      ...
      - **`logoUrl`** <mark style="color:green;">**string**</mark>
        ...
        **인증 창에 표시할 로고 URL**
        ...
        HTTPS가 아닐 경우 로고가 표시되지 않을 수 있습니다.
    </ParamTree>
</ParamTree>
