...
- 페이먼트월 프로젝트는 반드시 페이먼트월에서 제공한 포트원 전용 링크를 통해 생성해야 합니다. 정상적으로 생성된 프로젝트는 프로젝트명 뒤에 `[import]`가 표시됩니다.
- 페이먼트월을 통한 결제인 경우 [페이먼트월 배송 정보 등록 API](/api/rest-v2/pgSpecific#post%20%2Fpaymentwall%2Fdelivery%2Fconfirm)를 반드시 연동해야 합니다. 등록된 배송 정보는 차지백 발생 시 고객사의 상품 배송 완료 증빙 자료로 활용됩니다.
- 현재 요청 통화와 승인 통화(프로세싱 통화)가 다른 경우 전체 취소만 가능하며, 부분 취소는 지원되지 않습니다.
