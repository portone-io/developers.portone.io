---
title: 이니시스 통합인증
description: 이니시스 통합인증 연동 방법을 안내합니다.
targetVersions: ["v2"]
versionVariants:
...
SMS를 통한 본인인증을 진행하기 위해서는 이니시스와 별도 계약이 필요합니다.
...
포트원에서는 `bypass` 파라미터를 통해 이니시스에서 특수하게 지원하는 파라미터들을 입력할 수 있도록 지원하고 있습니다.
...
- `logoUrl`: 인증 창 좌측 상단 이니시스 로고 대신 들어갈 로고의 URL입니다
...
`SMS`의 경우, 이니시스와 계약하지 않으면 `알 수 없는 오류가 발생했습니다.` 메시지와 함께 인증 진행이 불가능합니다.
