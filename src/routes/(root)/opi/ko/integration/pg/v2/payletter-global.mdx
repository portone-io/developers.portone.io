...
- **일반 결제**
  - 특정 결제 수단만 결제창에 표기하고 싶으신 경우, `bypass.payletter_global.pginfo` 파라미터를 결제수단에 따라 아래와 같이 설정하시면 됩니다.
    ...
    - 알리페이 : `ICBAlipay`
    - 알리페이 HK (알리페이+) : `AlipayPlusAlipayHK`
    - BPI (알리페이+) : `AlipayPlusBPI`
    - Dana (알리페이+) : `AlipayPlusDana`
    - GCash (알리페이+) : `AlipayPlusGcash`
    - Rabbit LINE Pay (알리페이+) : `AlipayPlusRabbitLinePay`
    - Touch 'n Go (알리페이+) : `AlipayPlusTNG`
    - TrueMoney (알리페이+) : `AlipayPlusTrueMoney`
    - 해외카드 (미국) (MoR) : `CodaPayCreditCard_US`
    - 해외카드 (일본) (MoR) : `CodaPayCreditCard_JP`
    - 구글페이/애플페이 (일본) (MoR) : `CodaPayGoogleApplePay_JP`
    - PayPay (일본) (MoR) : `CodaPayPayPay`
...
- MoR 결제수단 중 일본 구글페이/애플페이(`CodaPayGoogleApplePay_JP`)의 경우 구글페이 혹은 애플페이를 직접 지정할 수 없고 제휴사에서 자체적으로 환경에 알맞은 결제 수단을 노출합니다.
- MoR 결제수단 중 일본 결제수단들(`CodaPayCreditCard_JP`, `CodaPayGoogleApplePay_JP`, `CodaPayPayPay`)의 경우 제휴사 정책으로 API를 통한 결제 취소가 불가능하며 페이레터 어드민 페이지를 통한 수동 취소만 가능합니다.
