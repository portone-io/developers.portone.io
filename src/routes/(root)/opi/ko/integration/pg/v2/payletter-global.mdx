...
- **결제창 일반 결제**
  ...
  페이레터의 경우, `payMethod` 파라미터를 지원하고 있지 않습니다.
  `payMethod` 파라미터를 입력하지 않으시면, 계약된 모든 결제 수단이 페이레터 결제창에 노출됩니다.
...
페이레터 연동만을 위한 파라미터는 [브라우저 SDK 파라미터 목록](/sdk/ko/v2-sdk/payment-request?v=v2#bypass-oneof-object)의 `bypass.payletter_global` 항목을 참조해 주세요.
...
- **빌링키 발급 및 결제**
  ...
  페이레터의 경우, `billingKeyAndPayMethod` 파라미터를 지원하고 있지 않습니다.
  `billingKeyAndPayMethod` 파라미터를 입력하지 않으시면, 계약된 모든 빌링키 발급 및 결제 수단이 페이레터 결제창에 노출되고,
  특정 결제 수단만 노출하고 싶으신 경우, 아래에 후술될 `bypass.payletter_global.pginfo` 파라미터를 사용하시면 됩니다.
