...
- 테스트 채널로 결제하기 위해서는 헥토파이낸셜 측 방화벽 설정이 필요합니다. 결제 테스트를 진행하는 회사 IP를 tech.support@portone.io로 전달해 주시면, 헥토파이낸셜 측에 IP 허용을 요청해 드립니다. 실연동 채널을 통한 결제는 이와 무관하게 정상적으로 이루어집니다.
