...
브랜드페이의 카드·계좌를 이용한 정기결제는 [브랜드페이 정기결제](/opi/ko/integration/pg/v1/toss-brandpay/billing) 문서를 참고하세요.
...
- `methodId`는 [`IMP.loadModule()`](/opi/ko/integration/pg/v1/toss-brandpay/module)을 호출 후 사용할 수
  있는, `getPaymentMethods()` 함수를 통해 확인할 수 있습니다.

<Tabs>
  <Tabs.Tab title="바로 결제">
    ```ts title="Javascript SDK"
    IMP.request_pay(
      {
        channelKey: "{콘솔 내 연동 정보의 채널키}",
