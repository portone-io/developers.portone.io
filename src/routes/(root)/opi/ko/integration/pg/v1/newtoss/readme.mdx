...
targetVersions: ["v1"]
versionVariants:
  v2: /ko/integration/pg/v2/tosspayments
---
...
<Tabs>
  <Tabs.Tab title="인증결제창 요청">
    ...
    <Details>
      <Details.Summary>기타 <strong>파라미터 설명</strong></Details.Summary>
  ...
  <Tabs.Tab title="비인증 결제창 요청">
    ...
    **빌링키(customer\_uid)로 결제 요청하기**
    ...
    빌링키 발급이 성공하면 실 빌링키는 customer\_uid 와 1:1 매칭되어 **포트원 서버에 저장**됩니다. customer\_uid를 고객사 내부서버에 저장하시고 [<mark style="color:blue;">**비 인증 결제요청 REST API**</mark>](/api/rest-v1/nonAuthPayment?v=v1#post%20%2Fsubscribe%2Fpayments%2Fagain)를 호출하시면 결제를 발생시킬 수 있습니다.
...
<Tabs>
  ...
  <Tabs.Tab title="고정식 가상계좌 발급">
    ...
    - [API 방식](/api/rest-v1/vbank?v=v1#post%20%2Fvbanks)
    - 결제창 방식
...
- [결제금액 사전등록 API](/api/rest-v1/payment.validation?v=v1#post%20%2Fpayments%2Fprepare)
- [결제취소 API](/api/rest-v1/payment?v=v1#post%20%2Fpayments%2Fcancel)
- [현금영수증 API](/api/rest-v1/receipt?v=v1)
- [현금영수증 발급(외부) API](/api/rest-v1/receipt?v=v1#post%20%2Freceipts%2Fexternal%2F%7Bmerchant_uid%7D)
