---
title: KG파이낸셜 (신모듈)
description: KG파이낸셜 (신모듈) 결제 연동 방법을 안내합니다.
targetVersions: ["v1"]
---
...
<Details>
  ...
  <Details.Content>
    - **과세**: 입력한 부가세와 상관없이 KG파이낸셜에서 자동 계산된 부가세 기준으로 처리됩니다.

    - **복합 과세**:
      - 첫번째 취소: 입력한 면세액, 부가세액 기준으로 처리됨
      - 두번째(마지막) 취소: 입력한 면세액, 부가세액과 상관없이 KG파이낸셜에서 자동 계산된 나머지 금액을 취소함
  </Details.Content>
</Details>
...
<Details>
  ...
  <Details.Content>
    ...
    면세액과 부가세 파라미터 사용 시 올바르게 계산된 금액을 입력해야 합니다. 그렇지 않으면 결제가 실패하거나 KG파이낸셜 측에서 계산된 금액이 적용되어 의도하지 않은 면세액, 부가세 정보로 거래가 발생할 수 있습니다.
  </Details.Content>
</Details>
...
<Details>
  <Details.Summary>가상계좌 환불 처리</Details.Summary>

  <Details.Content>
    KG파이낸셜 가상계좌 환불은 **매주 목요일 일괄 입금** 처리됩니다.

    - **화요일까지 접수된 요청**: 같은 주 목요일 입금
    - **수요일부터 접수된 요청**: 다음 주 목요일 입금
  </Details.Content>
</Details>
...
<Details>
  ...
  <Details.Content>
    - 아이폰 모바일 환경에서 크롬이나 인앱 브라우저에서 계좌이체를 하면 뱅크페이에서 결제 완료 후 **사파리로 이동하는 이슈**가 있습니다. (KG파이낸셜 측 수정 예정)
