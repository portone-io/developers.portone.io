...
<Details>
  <Details.Summary>빌링키 발급 수단 및 정보를 알 수 없음</Details.Summary>

  <Details.Content>
    페이팔을 통해 다양한 수단으로 빌링키 발급이 가능하지만, 페이팔이 승인 된 빌링키 발급 수단 및 정보를 알려주지 않습니다. 따라서 페이팔 빌링키 발급 건의 결제 수단(pay\_method)은 모두 paypal로 일괄 저장되며 [발급 된 빌링키 정보 조회](/api/rest-v1/billingkey?v=v1#get%20%2Fsubscribe%2Fcustomers%2F%7Bcustomer_uid%7D)시 카드 정보는 모두 null로 내려갑니다.
...
<Details>
  <Details.Summary>Magnes & Fraudnet 이상 거래 대응 필요</Details.Summary>
...
<Details>
  ...
  <Details.Content>
    ...
    ### `bypass.paypal_v2.enable-funding` / `bypass.paypal_v2.disable-funding`

    렌더링할(또는 제외할) 페이팔 버튼 종류를 comma(,) separated string으로 지정합니다. 사용 가능한 버튼 종류는 [페이팔에서 제공하는 문서](https://developer.paypal.com/sdk/js/configuration/#enable-funding)를 참고하세요.

    - 빌링키 발급에서는 카드 게스트 결제가 지원되지 않아 `card` 버튼은 항상 제외되며, `disable-funding`에 전달한 값은 `card`와 병합됩니다.
    - 페이팔 정책상 빌링키 발급(billing agreement)에서는 Pay Later 버튼과 Venmo가 지원되지 않습니다. `enable-funding`으로 지정해도 렌더링되지 않을 수 있습니다.
    - 미국 등 일부 국가에서 접속하면 PayPal 버튼 아래에 PayPal Credit 버튼이 함께 렌더링됩니다. 이를 제외하려면 `disable-funding`에 `credit,paylater`를 전달하세요.
    ...
    <div class="items-center gap-2 md:flex">
      <Figure src="/gitbook-assets/ko/paypal-rt/paypal-rt-no-shipping-address.png" caption="shipping_address를 전달하지 않은 경우 가입시 설정된 기본 정보(수령인 이름: John Doe / 배송 주소: 1 Main St, San Jose, CA 95131, UnitedStates)로 설정됩니다." />

      <Figure src="/gitbook-assets/ko/paypal-rt/paypal-rt-shipping-address.png" caption="shipping_address에 수령인 이름과 배송 정보를 전달한 경우 수령인 이름과 배송 정보가 override 됩니다." />
    </div>
  </Details.Content>
</Details>
