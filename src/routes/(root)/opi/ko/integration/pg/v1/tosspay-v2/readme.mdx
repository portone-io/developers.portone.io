...
<Tabs>
                ...
                <Parameter.Details>
                  - encryptedUserCi?: string
                    ...
                    **암호화된 사용자 CI**
                    ...
                    빌링키 발급 시 사용자 CI를 암호화하여 전달할 수 있습니다.
                </Parameter.Details>
            </Parameter.Details>
        </Parameter>
      </Details.Content>
  ...
  <Tabs.Tab title="비인증 결제 API">
    **빌링키로 결제 요청하기**
    ...
    빌링키 발급 시 전달한 `customer_uid`를 이용해서 빌링키 결제([**POST /subscribe/payments/again**](/api/rest-v1/nonAuthPayment?v=v1#post%20%2Fsubscribe%2Fpayments%2Fagain)) REST API를 다음과 같이 호출합니다.
    ...
    await fetch("https://api.iamport.kr/subscribe/payments/again", {
      ...
      body: JSON.stringify({
        ...
        name: "주문명", // 주문명
        cash_receipt_type: "person", // 현금영수증 발급 타입
        bypass: {
          tosspay_v2: {
            cashReceipt: true, // 현금영수증 발급 여부 (default: true)
            cashReceiptTradeOption: "GENERAL", // 현금영수증 발급 대상 타입
