...
본인인증 요청 및 응답 파라미터는 [브라우저 SDK 문서](/sdk/ko/v2-sdk/readme)를 참조해 주세요.
...
쿼리 문자열로 전달되는 내용은 [브라우저 SDK 응답 형식](/sdk/ko/v2-sdk/identity-verification-response)과 동일합니다.
...
- `gender`: 성별
  - 다날의 경우 항상 제공.
  - KCP의 경우 항상 제공.
  - KG이니시스의 경우 검증되지 않은 정보가 제공됩니다. `FRGNDInfo` 파라미터를 `N`으로 설정한 경우 제공되지 않습니다. [참고](/opi/ko/integration/pg/v2/inicis-unified-identity-verification#kg이니시스-특수-파라미터-안내)
