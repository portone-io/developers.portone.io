...
V2 SDK가 제공되지 않는 플랫폼(Flutter 등)이나,
...
1. `redirectUrl`에 결제 완료 시 결과를 전달받을 URL을 지정합니다. 결제 내에서 사용하지 않을 URL이면 됩니다.
2. `appScheme`에 외부 앱(카드사, 은행 등)에서 고객사 앱으로 복귀하기 위한 앱 scheme을 지정합니다.
...
`redirectUrl`에 결제 완료 시 결과를 전달받을 URL을, `appScheme`에 외부 앱에서 복귀하기 위한 앱 scheme을 지정합니다.
