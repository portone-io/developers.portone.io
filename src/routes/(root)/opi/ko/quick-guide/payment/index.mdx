  ...
  <Section section="client:request-payment">
    ...
    <ParamTree>
      ...
      **결제 금액**
      ...
      결제를 원하는 통화(`currency`)별 scale factor(소수점 아래 몇 번째 자리까지 유효한지)를 고려한 정수값만 허용됩니다.

      - 1000 만큼 원화(KRW) 결제를 하는 경우, scale factor가 0이기 때문에 **1000 \* (10의 0제곱) = 1000**을 전달해야 합니다.
      - 1.50 만큼 달러(USD) 결제를 하는 경우, scale factor가 2이기 때문에 **1.50 \* (10의 2제곱) = 150**을 전달해야 합니다.
      - `totalAmount`는 `currency`에 따라 실제 결제금액으로 변환되기 때문에 반드시 `currency`를 입력해야 합니다.

      #### currency: string

      **결제 통화**

      [ISO 4217](https://www.iso.org/iso-4217-currency-codes.html)에 의해 표준화된 통화 코드를 입력합니다.
      ...
      **에스크로 결제 여부**
      ...
      <Condition when={({ pg: { name }}) => name === "toss"}>
        - 에스크로를 사용하기 위해서는 토스페이먼츠와 별도 계약이 필요합니다.
      </Condition>

      <Condition when={({ pg: { name }}) => Array().includes(name)}>
        #### locale?: string
