...
i'mport provides payment window integration through JavaScript SDK
so that anyone can easily implement complex PG payment integration specifications.
...
## Change to PGs easily

Changes to PGs and payment methods are also processed simply
by changing the relevant parameter values.
...
## Provides samples and guide

We provide all payment references and integration samples and
guides for various programming languages in sync with the latest trends.
...
## Protect all payment data

All payment data is protected in real time through monitoring
...
## Provides cause of payment failures

By storing payment progress cycles in real time,
you can accurately analyze the reason and cause of payment failures.

- When payment widow is active
- When payment window is closed by the user
- When payment is complete

## Cancel payments easily

You can easily cancel payments for any PG payment method.

- Regardless of the payment method,
  you can use the same cancellation API to cancel any PG payment transaction.

- We support full and partial cancellations.

- You no longer have to waste development resources for implementing
  specific cancellation integration for each payment method.

**PortOne offers three value propositions.**
