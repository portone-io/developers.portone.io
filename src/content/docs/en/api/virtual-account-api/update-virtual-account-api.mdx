...
<Swagger method="put" path="/vbanks/{imp_uid}" baseUrl="https://api.iamport.kr" summary="Updates the deposit expiration or amount for a virtual account that is pending deposit.">
<SwaggerDescription>
**imp_uid**

 must be specified. (Since there can be multiple virtual accounts pending payment deposit for a single merchant_uid, you can only update a virtual account based on the imp_uid)
...
<SwaggerParameter name="amount" type="integer">
**Amount**

</SwaggerParameter>
<SwaggerParameter name="vbank_due" type="integer">
**Account expiration**
