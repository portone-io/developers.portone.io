...
REST API ([POST
https://api.iamport.kr/users/getToken](https://api.iamport.kr/#!/authenticate/getToken)) to get an
...
> - Reissuance (after expiration): A new access token is issued. (Expiration: 30 minutes after
>   issuance)
> - Reuse (before expiration): Existing access token is reused. (Expiration: same as
>   before, but extended by 5 minutes if requested within 1 minute from the original expiration)
