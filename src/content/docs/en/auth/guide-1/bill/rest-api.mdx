...
Add input fields for card information as shown below. Create a hidden field to store the **customer\_uid** to send to the server upon form submission. For corporate cards (excluding cards issued under an employee's name), enter a _10-digit business registration number_ for the `birth` parameter. The following example calls a `POST` request for `/subscription/issue-billing` with input values and **customer\_uid** when you click the **Pay** button.
...
<Hint style="success">
**To use the i'mport REST API, you must first get an** [**access token**](../../../api/rest-api-access-token.md)**.**
