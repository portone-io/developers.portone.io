...
#### subscriptionsubscriptionYou can use the customer\_uid obtained through the [Get billing key](../../api/billing-key-api/api-1.md) or [Non-authenticated payment (one-time)](../../api/api/request-non-authenticated-payment-one-time-api.md) API to request a subscription or scheduled payment.
...
For information about the i'mport Webhook and how to set the callback URL, refer to the  [**i'mport webhook**](../../result/webhook.md) guide.
