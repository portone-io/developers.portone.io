...
For a virtual account refund,
you must sign up for the v**irtual account special service** provided by each PG.
...
app.post("/payments/cancel", async (req, res, next) => {
  try {
    ...
    Payments.find({ merchant_uid }, async function (err, payment) {
      ...
      /* Call PortOne REST API to request refund */
      const getCancelData = await axios({
        url: "https://api.iamport.kr/payments/cancel",
        method: "post",
        headers: {
          "Content-Type": "application/json",
          Authorization: access_token, // Access token from PortOne server
        },
        data: {
...
If the virtual account refund request is successful,
the refund amount will be deposited (from PG) into the specified refund account normally
in about one working day.
