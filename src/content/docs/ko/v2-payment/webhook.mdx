...
- `status`: 결제건의 상태입니다. 가능한 `status`의 종류는 아래와 같습니다.
  ...
  - `PayPending`: **결제 승인 대기** 상태가 되었을 때 (페이팔 연동 시 발생)
...
- `data`: 웹훅을 트리거한 이벤트의 실제 세부 내용입니다. `type` 에 따라 해당 필드의 스키마가 달라질 수 있으며, `type` 별 상세 스키마는 아래와 같습니다.
  ...
  - **빌링키 발급 관련**
    - `billingKey`: 포트원에서 채번한 빌링키 고유 아이디입니다.

<Hint style="info">
    **`data` 스키마에는 별도 안내 없이 필드가 추가될 수 있습니다. 따라서 `data` 필드를 decode 하실 때에는 unknown 필드가 존재할 경우 에러를 발생시키지 않고 무시하도록 코드를 작성해주세요.**
</Hint>


**웹훅 payload 예시 1**: `type`이 `Transaction.Cancelled` 일 때
```json
{
...
**웹훅 payload 예시 2**: `type`이 `BillingKey.Issued` 일 때
```json
{
...
포트원에서는 악의적인 행위자가 웹훅을 위조하여 보냈는지를 검사할 수 있도록 시크릿을 기반으로 signature를 만들어 Webhook Request Header에 함께 전달합니다.
또한 포트원에서 제공하는 SDK를 이용하면 이 signature를 손쉽게 verify할 수 있습니다.
...
시크릿을 발급하고 난 이후부터 포트원에서 보내는 웹훅에는 **해당 시크릿을 이용해 만들어진 signature 정보가 Request Header에 함께 전달**될 것입니다.
포트원에서 제공하는 Server SDK를 이용하면 고객사 서버에서 웹훅을 수신할 때 signature가 유효한지를 손쉽게 verify할 수 있습니다. 현재 제공되고 있는 Server SDK 목록은 아래와 같습니다.

- jvm-server-sdk [Fina TODO 링크 달기]
- js-server-sdk [Cosmo TODO 링크 달기]

Express에서 server-sdk 사용 예시 코드:
[Cosmo TODO]

혹은 server-sdk 사용을 원치 않으시거나 sdk가 지원되지 않는 언어를 이용중이시라면 signature를 verify하는 코드를 직접 작성하실 수 있습니다.
포트원의 웹훅 verification은 [standard-webhooks](https://github.com/standard-webhooks/standard-webhooks/blob/main/spec/standard-webhooks.md) 스펙을 준수하고 있으므로, 해당 스펙에서 안내된대로 verify 코드를 작성하시면 됩니다.
...
- 고객사 서버 코드에서 사용되고 있던 시크릿을 새로 발급한 시크릿으로 교체합니다. (시크릿은 기밀 정보이므로 어플리케이션 내에 하드코딩하지 않고 환경 변수로 관리하시는 것을 권장합니다.)
[TODO 시크릿 만료 스샷 하나 첨부]
- 더 이상 기존 시크릿이 고객사 서버에서 사용되지 않는 것을 확인한 후, 콘솔에서 기존 시크릿을 만료시킵니다.

## 웹훅 요청 처리 코드 예시

다음은 포트원으로부터 보내진 웹훅 요청을 고객사 서버에서 받아 처리하는 예시 코드입니다.
본 코드는 참고용으로 작성된 코드이며, Server SDK를 이용한 signature verify 과정과 포트원 서버에서 결제 정보를 조회하여 검증 및 저장하는 과정을 포함하고 있습니다.
...
app.post("/portone-webhook", async (req, res) => {
  try {
    // [Cosmo TODO] verification code 여기도 첨부
