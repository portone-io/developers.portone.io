...
아래 코드는 API Secret을 이용해 accessToken을 먼저 발급하고, 이를 이용해 포트원의 [본인인증 내역 단건조회 API](/api/rest-v2/identityVerification#get%20%2Fidentity-verifications%2F%7BidentityVerificationId%7D "mention")를 호출하여 인증 정보를 조회하는 예시입니다.
  ...
  app.post("/identity-verifications", async (request, response) => {
    ...
    try {
      ...
      const verificationResponse = await axios({
        url: `https://api.portone.io/identity-verifications/${encodeURIComponent(identityVerificationId)}?storeId=${encodeURIComponent(storeId)}`,
        method: "get",
