...
emoji: ⌨
title: 다날 본인인증
description: 다날 본인인증 연동가이드를 확인합니다.
---
...
- [**다날 본인인증 설정**](../../ready/2-pg/payment-gateway/undefined#sms-본인인증) 페이지의 내용을 참고하여 PG 설정을 진행합니다.
...
- [본인인증 관련 API](../../api-v2/identity-verification) 페이지의 내용을 참고하여 연동을 진행합니다.
...
SDK의 `requestPayment` 함수의 파라미터 중 `bypass` 필드에 아래와 같이 값을 채울 수 있습니다.
...
[본인인증 요청 API](../../api-v2/identity-verification#-본인인증-요청-전송)를 사용할 경우, bypass 필드에 아래 Json을 Stringify 해서 입력하셔야 합니다.
