...
페이팔 결제 건은 승인 요청 시 바로 승인 되지 않고 일정 시간이후 처리되는 `승인 대기(pending)` 상태가 존재합니다.
때문에 트랜잭션 종료 시 콜백 함수로 전달되는 결제 아이디(paymentId)로 결제 내역을 조회한 후 응답 되는 status에 따라 후처리 로직을 구현해야 합니다. [[→ 결제내역 단건조회 API 바로가기](https://developers.portone.io/api/rest-v2/payment#get%20%2Fpayments%2F%7BpaymentId%7D)]

승인 대기 상태에서는 최종적으로 승인(`status : paid`)이 될 수도 있고 승인이 되지 않을 수도(`status : failed`) 있기 때문에 고객사는 반드시 **웹훅을 연동해야** 합니다. [[→ 웹훅 연동하기 바로가기](../../v2-payment/webhook)]

#### 결제 취소 후 취소 완료되지 않은 경우

결제 취소 시 취소 승인이 바로 되지 않고 일정 시간이후 취소 승인이 처리되는 경우가 존재합니다.
때문에 결제 취소에 대한 응답을 처리할 때 취소 승인이 완료되었는지 확인해야 합니다.

결제 취소 API를 통해 결제 취소한 경우 API 응답의 status와 cancel_history 값을 기준으로 취소 승인 여부를 판단해야 합니다.
status가 cancelled 이고 cancel_history에 취소 요청 내역이 있는 경우 취소가 승인된 것이고 그렇지 않은 경우 취소 승인대기 상태입니다.

콘솔을 통해 결제 취소 시 취소 승인이 바로 되지 않는 경우 결제 상태는 변경되지 않고, 콘솔 내 결제내역 상세에서 취소요청내역 탭에서 내역을 확인할 수 있습니다.
취소 요청이 승인대기 상태인 경우 최종적으로 승인(`status : paid`)이 될 수도 있고 승인이 되지 않을 수도(`status : failed`) 있기 때문에 고객사는 최종 취소 처리 결과를 전달받기 위해 고객사는 반드시 **웹훅을 연동해야** 합니다. [[→ 웹훅 연동하기 바로가기](../../v2-payment/webhook)]

<Details>
  <p slot="summary">결제 취소 후 취소 승인 대기상태 예시</p>
  <Figure src="/gitbook-assets/ko/image (3) (2).png" />
</Details>

<Details>
  <p slot="summary">결제 취소 후 취소 승인 대기상태일 때 결제내역 상세 예시</p>
...
<Details>
<p slot="summary">모든 결제 건의 결제수단은 paypal로 저장됩니다.</p>
...
<Details>
<p slot="summary">할부 결제 여부와 할부 개월수에 대한 정보를 제공하지 않습니다.</p>

Pay Later를 통해 할부 결제가 가능하지만 **결제 승인 이후 상세 할부 정보**를 알 수 없기에 제공하지 않습니다.

</Details>

<Details>
<p slot="summary">접속한 국가에 따라 렌더링 되는 결제 버튼이 다릅니다.</p>
...
<Details>
<p slot="summary">페이팔 회원과 비회원에 따라 결제 플로우가 상이합니다.</p>
