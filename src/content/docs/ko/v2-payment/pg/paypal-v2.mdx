...
페이팔의 경우 고객사의 결제 화면에 페이팔 결제 버튼을 렌더링 한 후 클릭해 페이팔의 결제창을 호출하는 방식입니다. `PortOne.requestPayment`와 기본적으로 동일한 파라미터를 받지만, 아래의 사항들이 기존 `PortOne.requestPayment`와 상이합니다.
- 일반결제 시 `uiType` 파라미터가 필수입니다.
- `payMethod`, `windowType` 파라미터를 입력할 필요가 없습니다.
- 결제 성공 시의 동작을 `onPaymentSuccess` 콜백 함수에, 결제 실패 시의 동작을 `onPaymentFail`에 작성하셔야 합니다.
...
페이팔의 경우 고객사의 결제 화면에 페이팔 빌링키 발급 버튼을 렌더링 한 후 클릭해 페이팔의 결제창을 호출하는 방식입니다. `PortOne.requestIssueBillingKey`와 기본적으로 동일한 파라미터를 받지만, 아래의 사항들이 기존 `PortOne.requestIssueBillingKey`와 상이합니다.
- 빌링키 발급 시 `uiType` 파라미터가 필수입니다.
- `billingKeyMethod`, `windowType` 파라미터를 입력할 필요가 없습니다.
