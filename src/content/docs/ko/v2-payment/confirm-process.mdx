...
import getNpmTypescriptExampleImage from "./_assets/v2/confirm_process_v2.png";

## 해당 서비스는 결제요청직전에 고객사 측에 결제의사를 최종적으로 확인하는 서비스입니다.

![](</gitbook-assets/ko/image (259).png>)

<Figure src={getNpmTypescriptExampleImage} caption="Confirm Process" />
...
Confirm Process 이용시 고객사는  `PortOne.requestPayment()`, `PortOne.loadPaymentUI()` 함수 호출 시 

**confirmUrl** 파라미터를 정의하고 해당 파라미터의 고객사 EndPoint URL을 설정합니다.
