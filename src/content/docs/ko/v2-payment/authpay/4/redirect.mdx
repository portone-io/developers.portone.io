...
함수의 반환 값을 사용할 수 없는 대신, redirectUrl 주소로 결과 값을 Query string(쿼리 스트링) 형태로 전달 받을 수 있습니다.
...
## 결제 검증 API를 이용해 결제 완료 처리하기

리디렉션 URL에 해당하는 페이지에서 쿼리 스트링으로 결과 정보를 받은 뒤, [3. 결제 검증 API 구현하기](../3 "mention")에서 구현한 API를 호출하여 결제가 올바르게 완료되었는지 확인해주세요.
