...
> >
> > **구매자 전체 이름** `fullName`과 `firstName` / `lastName`이 모두 입력된 경우 `fullName`으로
> > 기록됩니다.
>
> > **`firstName`** <mark style="color:green;">**string**</mark>
> >
> > **구매자 이름** `firstName`을 입력하는 경우 `lastName`도 필수로 입력해야 합니다. `fullName`이
> > 없고, `firstName`과 `lastName`이 존재하는 경우 `{firstName} {lastName}`으로 저장됩니다.
>
