...
여러 건의 상품주문이 발송처리 시도되는 경우, 일부 주문건만 발송처리에 성공할 수 있습니다.

시도된 상품주문 중 발송처리에 성공한 상품주문 에 대한 최종 정보만 제공됩니다.
...
**발송처리 햘 거래건의 네이버페이 상품주문번호**

생략되면 imp_uid에 해당되는 모든 상품주문을 발송처리합니다.
...
<SwaggerParameter name="dispatched_at" type="Integer" required="true">
<mark style="color:red;">
**발송일**
</mark>
...
<SwaggerResponse status="200: OK" description="모든 상품주문 발송처리 성공">
<Tabs>
...
<SwaggerResponse status="207: Multi-Status" description="일부 상품주문 발송처리성공 & 일부주문 발송처리실패">
네이버페이에 발솓요청시 product_order_id 개별로 발송요청이 진행되어야 합니다. 일부 건만 성공하는 경우 207 응답이 내려갑니다.
...
<SwaggerResponse status="500: Internal Server Error" description="모든 상품주문 발송처리실패">
```ts
{
    // Response
}
...
[**https://api.iamport.kr/#!/naver/naverShipProductOrder**](https://api.iamport.kr/#!/naver/naverShipProductOrder)
</Hint>
