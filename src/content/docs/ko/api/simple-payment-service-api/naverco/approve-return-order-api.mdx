...
### 네이버페이 주문형 상품 반품승인 처리 API

<Swagger method="post" path="/payments/{imp_uid}/naver/approve-return" baseUrl="https://api.iamport.kr" summary="네이버페이 주문형 상품 반품승인 처리">
<SwaggerDescription>

네이버페이 상품주문들을 반품승인처리합니다.

주문상품의 종류가 여러 개인 경우, 한 건의 포트원 거래건(imp_uid)에 여러 개의 네이버페이 상품주문이 존재하며 일부 또는 전체를 반품승인 처리할 수 있습니다.

`product_order_id` 가 생략될 경우, imp_uid에 해당되는 모든 네이버페이 상품주문들을 반품승인 처리합니다.

반품시 배송비 제외, 혹은 추가 청구액이 필요한 경우 반품보류 처리(POST 반품요청상품 반품보류 처리 API) 사용

여러 건의 상품주문이 반품승인 처리 시도되는 경우, 일부 주문건만 반품승인 처리에 성공할 수 있습니다.

시도된 상품주문 중 <b>반품승인 처리에 성공한 상품주문</b>에 대한 최종 정보만 제공됩니다.
...
**반품승인 처리햘 거래건의 네이버페이 상품주문번호**

생략되면 imp_uid에 해당되는 모든 상품주문을 반품승인처리합니다.
...
<SwaggerResponse status="200: OK" description="모든 상품주문 반품승인처리 성공">
<Tabs>
...
<SwaggerResponse status="207: Multi-Status" description="일부 상품주문 반품승인 처리성공 & 일부주문 반품승인 처리실패">
네이버페이에 반품승인 처리시 product_order_id 개별로 반품승인 처리이 진행되어야 합니다. 일부 건만 성공하는 경우 207 응답이 내려갑니다.
...
<SwaggerResponse status="500: Internal Server Error" description="모든 상품주문 반품승인 처리실패">
```ts
{
    // Response
}
...
[**https://api.iamport.kr/#!/naver/naverApproveReturnProductOrder**](https://api.iamport.kr/#!/naver/naverApproveReturnProductOrder)
</Hint>
