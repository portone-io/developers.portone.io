...
<Swagger method="post" path="/subscribe/payments/schedule" baseUrl="https://api.iamport.kr" summary="결제 예약(Schedule) API">
  <SwaggerDescription>
    ...
    예약건 별로 고유의 merchant\_uid를 전달해주셔야 합니다.
  </SwaggerDescription>
...
>   (스마트로(신모듈)의 경우, 특수문자를 포함하여 요청하는 경우 에러가 리턴되며 결제창이 호출되지 않습니다.)
>
> - `schedule_at` : 결제요청 예약시각(초) (UNIX timestamp in seconds)
>
> - `currency` : 결제통화 e.g.) KRW, USD, ...
>   (단, 페이팔(SPB/RT)의 경우 KRW 결제가 불가능하므로 반드시 유효한 값을 필수로 입력해야합니다.)
>
> - `amount` : 결제금액
...
>
> - `vat_amount`: amount 중 부가세(기본값: null). (나이스페이먼츠(구모듈/신모듈)), KG이니시스,
>   웰컴페이먼츠에서 사용가능 합니다.)
>
> - `name` : 주문명(누락되면 포트원 자체 기본값 사용)
>
> - `buyer_name` : 주문자명 (KSNET, 스마트로(신모듈), 웰컴페이먼츠 필수)
>
> - `buyer_email` : 주문자 Email (스마트로(신모듈) 필수)
>
> - `buyer_tel` : 주문자 전화번호 (스마트로(신모듈) 필수)
>
> - `buyer_addr` : 주문자 주소
>
...
> **pg 구분코드**
>
> 신규 customer\_uid를 등록하는 경우에만 유효합니다.
>
> (기존에 등록된 customer\_uid로만 schedule 하는 경우에는 pg파라메터 적용되지 않습니다.)
>
...
> **\{PG사}.\{PG상점아이디}**
>
> - 나이스페이먼츠, 토스페이먼츠 2가지 PG설정이 되어있다면, pg 파라미터로 **nice** 또는 **tosspayments**로 구분 가능
