...
> * `vat_amount`: amount 중 부가세(기본값: null). (나이스페이먼츠, 이니시스, (신)나이스페이에서 사용가능 합니다.)
> * `name` : 주문명(누락되면 포트원 자체 기본값 사용)
> * `buyer_name` : 주문자명 (KSNET, 스마트로-신모듈 필수)
...
> * `interest_free_by_merchant` : 카드할부처리시, 할부이자가 발생하는 경우(카드사 무이자 프로모션 제외) 부과되는 할부이자를 가맹점이 지불하고자 PG사와 계약된 경우(default : false) (현재, 나이스페이먼츠, KSNET, (신)나이스페이만 지원됨)
> * `use_card_point` : 승인요청시 카드사 포인트 차감하며 결제승인처리할지 flag. PG사 영업담당자와 계약 당시 사전 협의 필요(default : false) (현재, 나이스페이먼츠, (신)나이스페이만 지원됨)
