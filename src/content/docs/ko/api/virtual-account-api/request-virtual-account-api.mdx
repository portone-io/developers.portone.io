...
(필수 : 이니시스, 나이스, 헥토파이낸셜(구 세틀뱅크), 스마트로 - 신모듈) 토스페이먼츠 - 신모듈, KSNET의 경우 적용되지 않고 가맹점명이 사용됨
</SwaggerParameter>
...
(필수 : 이니시스, 토스페이먼츠 - 신모듈, KSNET,스마트로 - 신모듈)
</SwaggerParameter>
...
> 면세금액
>
> 나이스페이먼츠, (신)나이스페이 전용, 상점아이디의 과세설정이 **지정금액방식(복합과세)** 으로 설정되어 있는 경우 필수값 입니다.
>
