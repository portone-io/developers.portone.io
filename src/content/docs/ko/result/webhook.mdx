...
> Webhook `POST` 요청의 본문은 다음의 정보를 포함합니다. <b>가맹점 서버는 해당 정보를 수신하고 반드시 [결제내역 단건조회 API](../api/payment-api/get-payment-by-impuid-api)를 통해 결제건을 조회하여 검증 후 저장할 수 있습니다.</b>
> 통신 상의 문제로 웹훅을 수신받지 못 한 경우에도 [결제내역 단건조회 API](../api/payment-api/get-payment-by-impuid-api)를 통해 결제건을 상태를 조회 후 결제건을 처리해주시길 바랍니다.
>
