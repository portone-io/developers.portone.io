...
<b>가맹점 서버는 웹훅을 수신하고 반드시 [결제내역 단건조회 API](../api/payment-api/get-payment-by-impuid-api)를 통해 결제건을 조회하여 검증 후 저장할 수 있습니다.</b>
네트워크 상의 문제로 웹훅을 수신받지 못 한 경우에도 [결제내역 단건조회 API](../api/payment-api/get-payment-by-impuid-api)를 통해 결제건을 상태를 조회 후 결제건을 처리해주시길 바랍니다.

웹훅 수신 후 가맹점에서는 정상 응답을 하였으나 네트워크 상의 문제로 웹훅의 response가 포트원 서버로 도달하지 않아 웹훅이 재발송 될 수 있으니, 멱등성을 보장하여 처리해주시길 권장드립니다.
...
포트원 웹훅은 다음의 세 가지 고정된 IP으로 요청이 생성됩니다. IP확인은 필수가 아니므로 필요하신 가맹점에서만 참고용으로만 사용해주시고
반드시 [결제내역 단건조회 API](../api/payment-api/get-payment-by-impuid-api)를 통해 결제건을 조회하여 검증 후 결제건을 처리해주세요.
