...
취소 요청을 하기 위해서 먼저 [<mark style="color:blue;">**REST API access token**</mark>](../../api/rest-api-access-token) 을 발급받습니다. 발급받은 액세스 토큰(**`access token`**)을 이용하여[**포트원 취소 API**](../../api/api-1/api) 를 호출하여 결제 취소를 요청합니다.
