      ...
      IMP.request_pay(
        ...
        (rsp) => {
          ...
          if (rsp.success) {
            ...
            console.log(rsp); // 결제 성공 응답 정보 확인
          } else {
            // 결제 실패 시
            // 추가적인 로직을 수행할 부분
            console.log("결제가 실패하였습니다.");
            console.log(rsp); // 결제 실패 응답 정보 확인
          }
        },
      );
    },
...
- 주문번호는 결제창 요청 시 항상 **고유 값**으로 채번하여 사용해야 합니다.
- 결제 완료 후 **결제 위변조** 대사 작업시 주문번호를 이용하여 검증이 필요합니다. 따라서 주문번호는 고객사 서버에서 **고유하게**(unique) 채번한 후 **DB에 저장**하는 것을 권고드립니다.
