...
<Tabs>
  <Tab title="결제완료/가상계좌 발급완료">
    ```url
    curl https://myservice.com/payments/complete?imp_uid=결제건을_특정하는_포트원_번호&merchant_uid=고객사_주문번호&imp_success=true
    ```
  </Tab>

  <Tab title="결제 실패">
    ```url
    curl https://myservice.com/payments/complete?imp_uid=결제건을_특정하는_포트원_번호&merchant_uid=고객사_주문번호&imp_success=false&error_code=에러_코드(현재_정리된_체계는_없음)&error_msg=에러_메시지
    ```
  </Tab>
</Tabs>
