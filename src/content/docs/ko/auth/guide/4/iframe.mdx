...
결제가 완료되면 반환되는 응답 객체([**rsp**](../../../sdk/javascript-sdk-old/))의 결제 성공 여부에 따라 처리 로직을 **callback** 함수에 작성합니다. 요청이 성공했을 경우에 **결제번호(imp\_uid)와 주문번호(merchant\_uid)를 서버에 전달**하는 로직을 위와 같이 작성합니다.

<Hint style="info">
[payrt.md](../../../sdk/javascript-sdk/payrt.md "mention")에서 callback 함수로 전달되는 응답 파라미터를 확인할 수 있습니다.
