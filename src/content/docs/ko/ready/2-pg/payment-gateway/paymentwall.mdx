...
1. [페이먼트월 콘솔](https://api.paymentwall.com/developers/applications)
   → My Projects에서 해당 프로젝트 선택 → Widgets
...
이를 수신하기 위해서는 [페이먼트월이 제공하는 파라미터](https://docs.paymentwall.com/reference/pingback/custom-parameter#parameter-with-value-paymentwall-provides)를
핑백 파라미터로 추가해야 합니다.

1. [페이먼트월 콘솔](https://api.paymentwall.com/developers/applications)
   → My Projects에서 해당 프로젝트 선택 → Settings

2. _Custom Pingback parameters_ 항목으로 이동합니다.
...
```json
{
  ...
  "vbank_num": null,
  "extension": {
      "country_code": "KR", // 사용자의 국가 코드(alpha-2)
      // 중략
  }
}
```
