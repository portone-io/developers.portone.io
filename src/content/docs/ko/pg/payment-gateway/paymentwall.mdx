...
![](</gitbook-assets/ko/Paymentwall\_logo\_dark\_latest.jpeg>)

### 2.결제 요청하기

[JavaScript SDK](../../sdk/javascript-sdk-old/) IMP.**request\_pay**(param, callback)을 호출하여 페이먼트월 결제창을 호출할 수 있습니다. **결제결과**는 PC의 경우 IMP.request\_pay(param, callback) 호출 후 <mark style="color:red;">**callback**</mark> 으로 수신되며 모바일의 경우<mark style="color:red;">**m\_redirect\_url**</mark> 로 리디렉션됩니다.
...
**`paymentwall.{project`**_**`key}`** 형태로 기재(project\_key는 페이먼트월 관리자페이지에서 확인 가능)_
