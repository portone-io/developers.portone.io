...
emoji: ⌨
title: (신)나이스페이
description: (신)나이스페이 결제창 연동 방법을 안내합니다.
---
...
[**(신)나이스페이 설정**](../../../ready/2-pg/payment-gateway/nice-v2) 페이지의 내용을 참고하여 PG 설정을 진행합니다.

아래 기능을 사용하시려면 (신)나이스페이에 사전 신청 후 계약이 완료되어야 합니다.
...
(신)나이스페이 신 모듈 결제는 최신 SDK에서만 지원되는 기능입니다.
...
<Hint style="info">
  **(신)나이스페이 신모듈을 연동하기 위해서는 위에 안내된 JS SDK를 이용하셔야 합니다**
...
[JavaScript SDK](../../../sdk/javascript-sdk/readme) `IMP.request_pay(param, callback)`을 호출하여 (신)나이스페이 결제창을 호출할 수 있습니다.
...
주의: (신)나이스페이 일반결제시 주문 번호에 특수문자 사용 불가
...
주의: (신)나이스페이 빌링키 발급시 필수 입력으로 입력 길이는 **20자**로 제한됩니다.
...
(신)나이스페이의 경우 모바일 환경에서 **리디렉션 방식으로 빌링키 발급창이 렌더링 되기 때문에 필수입력입니다.**
