...
(신) 나이스페이먼츠의 가상계좌 결제 시 필수 입력해야합니다. 입금 기한은 아래 형식으로 입력해야 합니다.
- YYYY-MM-DD HH:mm:ss
- YYYYMMDDHHmmss
- YYYY-MM-DD
- YYYYMMDD

날짜만 입력한 경우 입력한 날짜의 23시 59분으로 고정됩니다.

ex) `vbank_due : 2024-02-01` 로 호출 시 입금기한은 `2024년 2월 1일 23시 59분까지` 입니다.
...
- 모바일 (팝업) 
  ...
  **나이스페이먼츠를 통한 알리페이 결제 시 모바일 환경에서는 팝업 방식만 지원됩니다. 리디렉션 방식으로 결제 시 정상 승인 되지 않을 수 있으니 이 점 유의하시길 바랍니다.**
