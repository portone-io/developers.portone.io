...
페이팔 결제 건은 승인 요청 시 바로 승인 되지 않고 일정 시간 후 처리되는 `승인 대기(pending)` 상태가 존재합니다. 따라서 가맹점은 트랜잭션 종료시 콜백 함수로 전달되는 아임포트 번호(imp_uid)로 결제 내역을 조회([GET /payments/\{imp_uid}](../../api/api-1/api-1))한 후 응답 되는 status를 보고 각 상황에 맞는 후 처리 로직을 작성해야 합니다.
