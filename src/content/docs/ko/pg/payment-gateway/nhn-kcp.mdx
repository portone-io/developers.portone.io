...
<Tab title="빌링키 발급 생년월일 고정">
```javascript
IMP.request_pay({
  ...
  bypass : {
    batch_birth_day_yn: "Y", // 생년월일입력 박스 렌더링시 고정 활성화
  }
  ...
});
```

NHN KCP 결제창을 이용한 빌링키 발급시 PC환경에서 공인인증서 절차를 생략하는 경우
카드정보 입력화면에서 생년월일 입력박스를 고정시킬수 있습니다.
해당 파라미터를 설정하지 않고 빌링키 발급 진행시 최초 주민등록번호 입력박스가 카드정보 두번째 항목 입력시 생년월일 입력박스로 변환됩니다.
