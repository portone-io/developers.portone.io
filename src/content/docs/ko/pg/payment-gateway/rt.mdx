...
emoji: ⌨
title: 페이팔 RT 빌링키 발급
description: 페이팔 RT(Reference Transaction) 빌링키 발급 연동 방법을 안내합니다.
---
...
<Hint style="warning">
페이팔 RT 빌링키 발급은 **SDK 1.3.0 부터 사용 가능**합니다.
...
위 JS SDK 를 이용하여 토스페이먼츠,케이에스넷,페이팔 RT 연동시 callback Data는
...
페이팔 RT 빌링키 발급은 가맹점 체크아웃 페이지에 페이팔 RT 버튼(아래 이미지 참고)을 렌더링 한 후, 이 RT 버튼을 클릭해 페이팔 빌링키 발급 창을 호출하는 방식입니다.

<Figure src="/gitbook-assets/ko/paypal-rt/btn-paypal-rt.png" />

따라서 다른 PG사와 빌링키 발급 플로우가 상이하기 때문에 가맹점은 **체크아웃 페이지가 렌더링 되는 시점에** `IMP.request_pay` 함수 대신 **`IMP.loadUI`라는 별도의 함수를 호출해 페이팔 RT 버튼을 렌더링 해야**합니다.

<Figure
  src="/gitbook-assets/ko/paypal-rt/paypal-rt-flow.png"
  caption="페이팔 RT 빌링키 발급 연동 플로우"
/>
...
페이팔 RT 동작의 특성상, 가맹점 체크아웃 페이지가 렌더링 될 때 빌링키 발급 요청 데이터가 결정 되어야 합니다.
...
하지만 페이팔 RT 때문에 페이지를 하나 더 만드는 것은 가맹점 입장에서 매우 번거로운 일이기 때문에 포트원에서는 처음 페이팔 버튼을 렌더링 시킨 후, 페이팔 버튼을 누르기 전 빌링키 발급 정보가 바뀌었을때 “빌링키 발급 요청 데이터”를 업데이트 할 수 있는 `IMP.updateLoadUIRequest` 함수를 제공하고 있습니다.

구매자 입력으로 인해 **빌링키 발급 요청 데이터가 변경될때 `IMP.updateLoadUIRequest`함수를 호출**하시면 구매자가 페이팔 RT 버튼을 누를때 최종적으로 변경 된 빌링키 발급 요청 데이터로 페이팔 빌링키 발급 창이 호출됩니다.
...
RT 기능을 사용하시기 위해 다음 정보를 확인하셔야 합니다.
