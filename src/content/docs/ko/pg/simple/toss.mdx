...
<Hint style="info">
  토스페이(tosspay)는 일반결제만 지원됩니다. 일반결제 및 정기결제 모두 사용하시는 고객사께서는 [토스페이(tosspay_v2) 연동 가이드](tosspay-v2/readme)를 참고하여 진행해주세요.
  신규 고객사의 경우 가급적 토스페이(tosspay_v2)로 연동하는 것을 권장드립니다.
</Hint>
