...
특정 고객 하위에 등록되어있는 결제수단의 변경이 필요한 경우 기존에 빌링키 발급시 이용한 `customer_uid`를 통해 새로운 빌링키를 발급받고 이 때 포트원에서는 기존에 저장되어있던 빌링키 정보를 파기합니다.

이는 대부분의 PG사/간편결제사들의 경우 결제수단 변경이라는 개념 없이 발급에 대한 개념만 존재하기 때문입니다.

다만 네이버페이의 경우는 변경에 대한 개념이 존재합니다.
네이버페이에서는 현재 본인 계정으로 발급/등록되어있는 정기결제 목록 확인이 가능하기에 결제수단 변경이 필요할 때 다른 PG사/간편결제사와 같이 신규 등록으로 처리하는 경우 파기한 이전의 결제수단 정보가 구매자의 네이버페이 계정에는 그대로 남아있는 문제가 발생합니다.

그렇기 때문에 포트원을 통해 네이버페이 빌링키 발급을 진행할 때 설정하는 `customer_uid` 값이 기존에 사용하던, 이전에 네이버페이 빌링키를 발급할 때 이용했던 값이고 결제수단 변경을 처리하는 경우
`naverActionType` 파라미터를 `CHANGE`로 설정해 기존 등록되어있는 결제 수단을 대체하여야 합니다.
