...
<Hint style="warning">
하위 상점을 연동할 경우 액세스 토큰을 발급받을 때 **대표 상점**의 **API Key** 와 **API Secret**을 사용해야 합니다.
...
<Hint style="warning">
하위 상점의 결제를 연동할 경우 포트원 API 호출시 `Tier` 헤더에 하위 하위상점 티어 코드를 입력해야 합니다.
...
- 한 고객사에서 여러대의 웹서버가 동시에 경쟁적으로 REST API(`/users/getToken`)를 호출하는 상황
- 한 고객사에서 여러대의 웹서버가 시간 동기화 되어있지 않은 상황
