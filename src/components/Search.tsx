...
export default function Search() {
  ...
  const extractTermsFromPage = (): SearchResult[] => {
    ...
    headings.forEach(heading => {
      ...
      for (const [sectionElement, sectionName] of sectionMap) {
        if (sectionElement.compareDocumentPosition(heading) & Node.DOCUMENT_POSITION_FOLLOWING) {
          section = sectionName;
          break;
        }
      }
  ...
  const scrollToTerm = (termId: string) => {
    ...
    if (element) {
      ...
      const header = document.querySelector('header') || document.querySelector('[data-header]') || document.querySelector('.header');
  ...
  return (
    <div class="mb-8 w-1/2 max-w-full">
      <div class="relative">
        <input
          type="text"
      ...
      {/* 검색 결과 */}
      {searchTerm().length > 0 && (
        <div class="mt-4 bg-white border border-gray-200 rounded-lg shadow-lg max-h-96 overflow-y-auto">
          {isSearching() ? (
            ...
            <div class="p-2">
              <div class="text-sm text-gray-600 mb-2 px-2">
              ...
              <For each={searchResults()}>
                {(result) => (
                  <button
                    ...
                    class="w-full text-left p-3 hover:bg-gray-50 rounded-md transition-colors duration-150 border-b border-gray-100 last:border-b-0"
                  >
                    <div class="font-medium text-gray-900 mb-1">{result.title}</div>
                    <div class="text-sm text-gray-600 mb-1">{result.section}</div>
